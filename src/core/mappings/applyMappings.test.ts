...
describe('applyMappings', () => {
  it('adds a suggestion for a term with no existing mapping', () => {
    ...
    expect(result['category:news']).toMatchObject({ targetTermId: 'c1', origin: 'suggested' });
  });

  it('overwrites a stale suggested mapping on recomputation', () => {
    const existing: Record<string, TermMapping> = {
      'category:news': { oldDomain: 'category', oldNicename: 'news', targetTableId: 'category', targetTermId: 'old-stale-id', origin: 'suggested', score: 0.6 },
    };
    const result = applyMappings(OLD_TERMS, NEW_TABLES, existing);
    expect(result['category:news'].targetTermId).toBe('c1');
  });

  it('never overwrites or removes a user-owned mapping, even if it no longer matches', () => {
    const existing: Record<string, TermMapping> = {
      'category:news': { oldDomain: 'category', oldNicename: 'news', targetTableId: 'post_tag', targetTermId: 'manually-chosen', origin: 'user' },
    };
    const result = applyMappings(OLD_TERMS, NEW_TABLES, existing);
  ...
  it('removes a suggested mapping that no longer matches anything', () => {
    const existing: Record<string, TermMapping> = {
      'category:news': { oldDomain: 'category', oldNicename: 'news', targetTableId: 'category', targetTermId: 'c1', origin: 'suggested', score: 0.9 },
    };
  ...
  it('leaves mappings for terms outside the current oldTerms list untouched', () => {
    const existing: Record<string, TermMapping> = {
      'post_tag:unrelated': { oldDomain: 'post_tag', oldNicename: 'unrelated', targetTableId: 'post_tag', targetTermId: 'x', origin: 'user' },
    };
    const result = applyMappings(OLD_TERMS, NEW_TABLES, existing);
