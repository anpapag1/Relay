...
- [x] **Add edit button on article metadata** — done: an "Edit article" button in the metadata panel of the article sidebar switches the preview to Edit mode.
...
- [ ] **Fix edit metadata button** — right now the "Edit article" button in the metadata panel switches the preview to Edit mode, but it doesn't allow editing of the metadata itself. The goal is to enable users to edit the article's metadata (title, author, date, etc.) directly from the metadata panel not to switch modes on the article preview.